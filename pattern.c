...
void transpattvec(const Pattern *p, double *vec)
{
	...
	vec[1] = vec[1] + p->yoff;
	vec[2] = 0.0;
	//printf("pat: %f, %f, %f ", vec[0], vec[1], vec[2]);
	matxvec((double *) p->trans, vec, temp);
	vec[0] = temp[0] + p->pos[0];
	vec[1] = temp[1] + p->pos[1];
	vec[2] = temp[2] + p->pos[2];
	//printf("glo: %f, %f, %f\n", vec[0], vec[1], vec[2]);
}

double getdist(const Pattern *p, const uint32_t *rgb, double pdist, int orien)
{
...
{
	// MIGHT NEED TO CHECK....
	int ind = (int) round(hue*p->relbins);

	if (ind == p->relbins)
		ind = 0;
	// Need to reverse relation depending on orientation.
	if (orien)
		return p->segsize*p->rel[ind];
	else
		return p->segsize*(1.0 - p->rel[ind]);
}
